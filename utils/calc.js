...
function Calc() {
    ...
    //TODO: check candles length agains indicator input length
  this.isIndicator = expression => {return !(typeof this[expression.split('_')[0]] === "undefined")},
  this.isCandleValue = expression => {return ['O', 'H', 'L', 'C', 'v'].includes(expression)},
  this.BB = (candles, args) => {return boll(getSources(candles, args[0]), args[1], args[2])[args[3]]},
  this.EMA = (candles, args) => {return ma.ema(getSources(candles, args[0]), args[1])},
